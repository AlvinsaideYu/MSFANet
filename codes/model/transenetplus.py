...
def make_model(args, parent=False):
    return TransENetplus(args)

class BasicModule(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, block_type='basic', bias=True, bn=False, act=nn.ReLU(True)):
        super(BasicModule, self).__init__()
        self.block_type = block_type
        m_body = []
        if block_type == 'basic':
            n_blocks = 10
            m_body = [common.BasicBlock(conv, n_feat, n_feat, kernel_size, bias=bias, bn=bn) for _ in range(n_blocks)]
        elif block_type == 'residual':
            n_blocks = 5
            m_body = [common.ResBlock(conv, n_feat, kernel_size) for _ in range(n_blocks)]
        else:
...
class TransENetplus(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(TransENetplus, self).__init__()
        self.args = args
