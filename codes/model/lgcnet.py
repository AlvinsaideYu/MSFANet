...
class LGCNET(nn.Module):
    def __init__(self, args, nfeats=32):
        ...
        self.conv7 = nn.Conv2d(nfeats * 2, nfeats, kernel_size=3, stride=1, padding=1, bias=True)
        self.upsample = nn.Upsample(scale_factor=self.scale, mode='bilinear', align_corners=False)
        self.conv_final = nn.Conv2d(nfeats, args.n_colors, kernel_size=3, stride=1, padding=1, bias=True)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        out = self.conv_final(out) + self.upsample(residual)
        return out
...
if __name__ == "__main__":
    ...
    x = torch.rand(1, args.n_colors, args.patch_size, args.patch_size)  # 输入图像大小
    x = x.cuda()
