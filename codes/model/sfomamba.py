...
class SFOMamba(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        super(SFOMamba, self).__init__()

        self.fenet = FENetModule(args, conv)
        self.srdd = SRDDModule(args, conv)
        self.omnisr = OmniSRModule(num_in_ch=args.n_colors, num_out_ch=args.n_colors, num_feat=128, upsampling=args.scale, window_size=8)  # 使用OmniSRModule
    ...
    def forward(self, x):
        ...
        # OmniSR output
        omnisr_out = self.omnisr(x)
        omnisr_out = nn.functional.interpolate(omnisr_out, size=(residual.size(2), residual.size(3)), mode='bicubic', align_corners=False)

        # Combine outputs and apply attention
        combined_out = fenet_out + srdd_out + omnisr_out
